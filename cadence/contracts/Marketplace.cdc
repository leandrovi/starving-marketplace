...
// Marketplace.cdc
//
// The Marketplace contract is a very basic sample implementation of an NFT Marketplace on Flow.
//
...
pub contract Marketplace {
    ...
    pub resource interface SalePublic {
        pub fun purchase(tokenID: UInt64, recipient: Capability<&AnyResource{StarvingNFT.StarvingChildrenNFTCollectionPublic}>, buyTokens: @TAPToken.Vault)
    ...
    pub resource SaleCollection: SalePublic {
        ...
        access(self) var ownerCollection: Capability<&StarvingNFT.Collection>
        ...
        access(account) let ownerVault: Capability<&AnyResource{TAPToken.Receiver}>

        init (ownerCollection: Capability<&StarvingNFT.Collection>, 
              ownerVault: Capability<&AnyResource{TAPToken.Receiver}>) {
        ...
        pub fun listForSale(tokenID: UInt64, price: UFix64) {
            pre {
                self.ownerCollection.borrow()!.getIDs().contains(tokenID):
    ...
    pub fun createSaleCollection(ownerCollection: Capability<&StarvingNFT.Collection>, 
                                 ownerVault: Capability<&AnyResource{TAPToken.Receiver}>): @SaleCollection {
